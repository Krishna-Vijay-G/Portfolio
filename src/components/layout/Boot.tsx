...
/** Stage timings: anagram beats, when the boot UI dresses in, when the emblem splits, and the fail-safe. */
const HOLD = 560;
const GLITCH = 460;
const MOVE = 860;
const LOCK = 640;
...
/** Staged boot overlay: emblem pops first, boot UI dresses in, curtain tears when the name resolves. */
export function Boot() {
  const { fx, setBootStage } = useUI();
  ...
  useEffect(() => {
    ...
    setLite(window.matchMedia('(max-width: 767px)').matches);
  }, [setBootStage]);
  ...
  }, [resolved, skip, setBootStage]);

  useEffect(() => {
    if (skip || resolved) return;
                  ...
                  >
                    ...
                    <span className="h-px flex-1 bg-white/12">
                      <motion.span
                        ...
                        animate={{ scaleX: 1 }}
                        transition={{
                          duration: (OVERTURE + NOMINAL - DRESS_AT) / 1000,
                          ease: 'easeInOut',
                        }}
                      />
                    </span>
...
/** One half of the emblem pair: the lead copy pops in alone, its twin shows itself only as the two part. */
function BootEmblem({
  dir,
  lead,
  split,
